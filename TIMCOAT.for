...
C  MSWITCH     I*4      I  : Run TIMCOAT in mode with MSWTICH = 1 or MSWITCH = 2 Pd and 
C                            fission product corrosion of SiC and kernel migration
...
C  Mode 2 adds Pd migration, corrosion (thinning) of the SiC layer, and the Amoeba effect.
     ...
     &                ' product corrosion of SiC?  (no = 1, yes = 2)'
      READ(IKEY,*) MSWITCH
C
C  Select the type of coolant to use:  He = 1, Flibe (Li2BeF4) = 2.
      WRITE(ITERM,*) 'Select coolant type: Helium = 1, Flibe = 2'
      READ(IKEY,*) CSWITCH
C
...
C         Write headings to stress, strains, and displacement output files
          ...
          WRITE(IOUTER,642, ADVANCE='NO') RADIUS
          WRITE(IOUTET,642, ADVANCE='NO') RADIUS
          WRITE(IOUTUR,629, ADVANCE='NO') RADIUS
          DO 127 K = 0, NDIVI+1
...
C    Calculate temperature distribution in particles
        CALL TEMPERATURE(QPPP, T_COOLANT, BURNUP, T_PARTICLE, CSWITCH)  !calculate T distribution
...
C    Note:  Diecker Thesis uses uses the term DPDn to refer to DCORR
      IF (MSWITCH .EQ. 2)  THEN
        DCORR = (255.2*DT/3600) *
...
C    Register end-of-life mean tangential stresses in layers
		  ...
		  SIGLSIC  = SIGTSIC
C
C    Evaluate fuel failure from Pd penetration in SiC
...
C    DPD = distance of Pd penetration
     ...
     &                     OPERTIME, DT, MD)
         END IF
C    With MSWITCH = 1, do not calculate Pd penetration
      ELSE
         DPD = 0.0D0
         CALL FAILURE(SIGR, SIGT, FAIL, FAILTYPE, DPD, N, 
     &                     OPERTIME, DT, MD)
...
C    Output a bunch of results to various output files
     ...
     &                         HCARD(J,18), HCARD(J,19)
	        WRITE(IOUTSR,629) HCARD(J,4)
	 	    WRITE(IOUTST,629, ADVANCE='NO') HCARD(J,4)
	        WRITE(IOUTER,642, ADVANCE='NO') HCARD(J,4)
	 	    WRITE(IOUTET,642, ADVANCE='NO') HCARD(J,4)
	        WRITE(IOUTUR,629, ADVANCE='NO') HCARD(J,4)
		    DO 280 K = 1, NDIV
	          WRITE(IOUTSR, 629, ADVANCE='NO') SIGR(K)
	          WRITE(IOUTST, 629, ADVANCE='NO') SIGT(K)
	          WRITE(IOUTER, 641, ADVANCE='NO') EPIR(K)
	          WRITE(IOUTET, 641, ADVANCE='NO') EPIT(K)
	          WRITE(IOUTUR, 629, ADVANCE='NO') UR(K)
280           CONTINUE
              WRITE(IOUTSR,*)
              WRITE(IOUTST,*)
              WRITE(IOUTER,*)
...
C    Register end-of-life mean tangential stresses in layers
		  ...
		  SIGLSIC  = SIGTSIC
C
C    If MSWITCH = 2, then calculate Pd penetration
...
C             Send stresses, strains, and displacement to output files
	        ...
	        WRITE(IOUTSR,629, ADVANCE='NO') HCARDB(J,4)
	        WRITE(IOUTST,629, ADVANCE='NO') HCARDB(J,4)
	        WRITE(IOUTER,642, ADVANCE='NO') HCARDB(J,4)
	        WRITE(IOUTET,642, ADVANCE='NO') HCARDB(J,4)
	        WRITE(IOUTUR,629, ADVANCE='NO') HCARDB(J,4)
              DO 778 K = 1, NDIV
	          WRITE(IOUTSR, 629, ADVANCE='NO') SIGR(K)
	          WRITE(IOUTST, 629, ADVANCE='NO') SIGT(K)
	          WRITE(IOUTER, 641, ADVANCE='NO') EPIR(K)
...
C    CSWITCH           Helium = 1, Flibe (Li2BeF4) = 2                *
...
C    CP_FLIBE [J/kg.K] D:  Flibe specific heat                         *
C    D_FLIBE  [kg/m^3] D:  Flibe density                               *
C    K_FLIBE  [W/m.K]  D:  Flibe thermal conductivity                  *
C    MU_FLIBE [kg/m.s] D:  Flibe viscosity                             *
C    VC_FLIBE [m/s]    D:  Flibe characteristic velocity               *
C    RE_FLIBE       D:  Flibe Reynolds number                          *
C    PR_FLIBE       D:  Flibe Prandtl number                           * 
C    H_COOLANT (W/m^2.K) D: Heat transfer coefficient of coolant       *
C                                                                      *
C  Others                                                              *
...
C  Helium thermal properties
	DOUBLE PRECISION HE_THERMAL, CP_HE, D_HE, MU_HE, K_HE, VC_HE,
     &                 RE_HE, PR_HE
C  Flibe (Li2BeF4) thermal properties
      DOUBLE PRECISION FLIBE_THERMAL, CP_FLIBE, D_FLIBE, MU_FLIBE, 
     &                K_FLIBE, VC_FLIBE, RE_FLIBE, PR_FLIBE
C  Others
        DOUBLE PRECISION H_COOLANT
...
C  Thermal properties for coolant He (from A. F. Mills)
      CP_HE = 5200.0D0                                                 ! J/kg.K at 7.5 MPa
	...
	IF (INDEX .EQ. 0) INDEX = INDEX + 1                            !exceeds lowerbound; make adjustment
     ...
     &        /(HE_THERMAL(INDEX+1,0)-HE_THERMAL(INDEX,0))
      ELSE
C  Thermal properties of coolant Flibe
      CP_FLIBE = 2390.0D0    ![J/kg.K]  specific heat capacity from Benes 2012 section 3.13.4.5.1 
      K_FLIBE = 0.0005D0*(T_COOLANT+273.15D0)+(32.0D0/33.0D0)-0.34D0    ![W/m.K]  from Ignatiev Correlation.  Eqn 6 from ORNL/TM-2006/12
      D_FLIBE = 2415.6D0 - 0.49072D0*(T_COOLANT+273.15D0)   ![kg/m^3] from Zaghloul Correlation.  Eqn 2.14 from INL/EXT-10-18297
      MU_FLIBE = (0.116D0*EXP(3755.0D0/(T_COOLANT+273.15D0)))/1000.0D0   ![Pa.s] = [kg/m.s] Eqn 6 from Benes 2012 
C
      END IF
...
C   Find K of fuel: UO2, UCO or else.
	...
	ELSE IF(FUELTYPE .EQ. 'UCO') THEN
     ...
     &           /(UCO_THERMAL(INDEX+1,0)-UCO_THERMAL(INDEX,0))        
      ELSE   !Temporarily use this value. Don't know K of other fuels yet.
	  K_FUEL = 2.30D0                                                ! W/m.K
	END IF
C   Find K of buffer
	...
	IF (INDEX .EQ. 0) INDEX = INDEX + 1     !exceeds lowerbound; make adjustment.
      IF (INDEX .EQ. 6) INDEX = INDEX - 1     !exceeds upperbound; make adjustment.
...
C   Find K of SiC
	...
	IF (INDEX .EQ. 0) INDEX = INDEX + 1     !exceeds lowerbound; make adjustment.
      IF (INDEX .EQ. 4) INDEX = INDEX - 1     !exceeds upperbound; make adjustment.
...
C   Heat transfer coefficient of He is calculated using Achenbach Correlation.
	...
	H_COOLANT = (K_HE/PEBDIAMETER)*(PR_HE**(1.0D0/3.0D0))*     ![W/m^2.K]
...
C   Need heat transfer coefficient for flibe in a packed bed.  Achenbach should be appropriate (250<Re<500000)
      ...
      PR_FLIBE = MU_FLIBE*CP_FLIBE/K_FLIBE                    !Prandtl number of flibe
      H_COOLANT = (K_FLIBE/PEBDIAMETER)*(PR_FLIBE**(1.0D0/3.0D0))*               ![W/m^2.K]
     &       (((1.18D0*RE_FLIBE**0.58D0)**4.0D0+
     &         (0.23D0*RE_FLIBE**0.75D0)**4.0D0)**0.25D0)
      END IF
...
C   Update pebble temperature profile
      T_PEBBLE(0) = TPEBBLE(T_COOLANT, H_COOLANT, Q_PFZ, 0.0D0)
      T_PEBBLE(1) = TPEBBLE(T_COOLANT, H_COOLANT, Q_PFZ, PFZRADIUS)
      T_PEBBLE(2) = TPEBBLE(T_COOLANT, H_COOLANT, Q_PFZ, PEBRADIUS)
C  Calculate the temperature profile of a particle randomly chosen in the pebble
      R = R_IN_PEBBLE
	T_PS = TPEBBLE(T_COOLANT, H_COOLANT, Q_PFZ, R)
...
C***********************************************************************
C                                                                      *
      FUNCTION TPEBBLE(T_COOLANT, H_COOLANT, Q_PFZ, R)
C
      DOUBLE PRECISION TPEBBLE, T_COOLANT, H_COOLANT, Q_PFZ, R
